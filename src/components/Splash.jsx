    ...
    <div className="splash">
      <div style={{ opacity: phase >= 1 ? 1 : 0, transform: phase >= 1 ? "none" : "translateY(30px)", transition: "all 1s cubic-bezier(0.16,1,0.3,1)", marginBottom: 8 }}>
        <div className="splash-logo">
          <span style={{ color: T.paper }}>CASE</span><span style={{ color: T.teal }}>ZERO</span>
        </div>
      </div>
...
export function LandingScreen({ onStart, hasKey }) {
  const features = [
    ["🔍","Detective Mode","Explore crime scenes"],["💬","AI Interrogation","Suspects respond to anything"],
    ["🧬","Mood System","Suspects shift emotionally"],["🔄","Dynamic Alibis","Stories change under pressure"],
    ["👁","Witness System","Call in what they saw"],["⚔","Cross-Examine","Break their contradictions"],
    ["⏱","Case Timer","Race the clock"],["🔬","Forensics Lab","Deep-analyze evidence"],
    ["🎙","AI Narrator","Cinematic atmosphere"],["🎯","Reverse Grill","You become the suspect"],
    ["📱","Mobile Companion","Play on any screen"],["🎓","Tutorial Mode","Learn as you play"],
  ];
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 24px 40px", textAlign: "center", background: `radial-gradient(ellipse 80% 60% at 50% 0%, ${T.teal}08, transparent)` }}>
        {!hasKey && (
          <div style={{ marginBottom: 20, maxWidth: 500, width: "100%" }}>
            <div className="api-warn anim-up">
              ...
              <div style={{ fontSize: 12, color: T.inkSec }}>Add your <strong style={{ color: T.teal }}>OpenAI API key</strong> in Settings before playing.</div>
            </div>
          </div>
        )}
        ...
        <p className="noir anim-up" style={{ fontSize: "clamp(15px,2vw,20px)", color: T.inkSec, marginBottom: 40, letterSpacing: "0.06em", animationDelay: "0.1s" }}>
        ...
        <div className="anim-up" style={{ display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center", marginBottom: 56, animationDelay: "0.15s" }}>
          ...
          <button className="btn btn-gold btn-lg" style={{ fontSize: 15 }} onClick={() => onStart("tutorial")}>🎓 TUTORIAL</button>
          <button className="btn btn-ghost btn-lg" style={{ fontSize: 14 }} onClick={() => onStart("settings")}>⚙ SETTINGS</button>
        </div>
        <div className="anim-up" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 10, maxWidth: 760, width: "100%", animationDelay: "0.2s" }}>
          {features.map(([icon, title, desc]) => (
            <div key={title} style={{ background: T.void, border: `1px solid ${T.smoke}`, borderRadius: 6, padding: "14px 14px 12px", textAlign: "left", transition: "border-color 0.2s", cursor: "default" }}
