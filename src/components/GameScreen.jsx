...
import { T, DIFFICULTY, getMood, MOODS } from '../tokens';
...
export function GameScreen({ gameState, settings, onEnd, isTutorial = false }) {
  ...
  const [clues, setClues] = useState(() => {
    ...
    if (diff.freeClues > 0) {
      let g = 0;
      c = c.map(x => { if (!x.found && x.critical && g < diff.freeClues) { g++; return { ...x, found: true }; } return x; });
    }
    return c;
  });
  ...
  const triggerNarrator = async (ph) => {
    ...
    const pr = `Case: ${caseData.title}. Phase: ${ph}. Clues found: ${fc}. One atmospheric narrator line.`;
  ...
  const getHint = async () => {
    ...
    const h = await callAI(
      `Detective found: ${foundClues.map(c => c.name).join(",") || "nothing"}. One cryptic noir hint ≤20 words pointing toward the next critical clue.`,
  ...
  // ── Detect TV / mobile ───────────────────────────────────
  const isTV = typeof window !== "undefined" && window.innerWidth >= 1400;
    ...
    <div style={{ minHeight: "100vh", paddingBottom: isMobile ? 80 : 0 }}>
      ...
      <div className="top-nav">
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {!settings.openaiKey && <span className="tag tag-red" style={{ fontSize: 9 }}>NO API KEY</span>}
        </div>
      ...
      {/* MAIN LAYOUT — TV: 3-panel, Desktop: 2-panel, Mobile: single */}
      {isTV ? (
        <TVLayout
          caseData={caseData} clues={clues} phase={phase} setPhase={setPhase}
          foundClues={foundClues} progress={progress} activeRoom={activeRoom}
          setActiveRoom={setActiveRoom} discoverClue={discoverClue} notes={notes}
          setNotes={setNotes} settings={settings}
          suspects={caseData.suspects} selSuspect={selSuspect} setSelSuspect={setSelSuspect}
          interrogHist={interrogHist} setInterrogHist={setInterrogHist}
          questionCounts={questionCounts} setQuestionCounts={setQuestionCounts}
          dynamicAlibis={dynamicAlibis} setDynamicAlibis={setDynamicAlibis}
          lieScores={lieScores} setLieScores={setLieScores}
          crossState={crossState} setCrossState={setCrossState}
          witnessState={witnessState} setWitnessState={setWitnessState}
          subTab={subTab} setSubTab={setSubTab}
          player={player} diff={diff}
          revSuspicion={revState.suspicion}
          hint={hint} hintUsed={hintUsed} hintLoading={hintLoading} showHint={showHint}
          getHint={getHint} unlimitedHints={diff.unlimitedHints} aiHints={settings.aiHints}
          setShowDossier={setShowDossier} setShowTimeline={setShowTimeline}
          gameMode={gameMode}
        />
      ) : (
        <StandardLayout
      ...
      {showReverse && <ReverseModal caseData={caseData} player={player} state={revState} setState={setRevState} onSubmit={() => {}} onClose={() => setShowReverse(false)} diff={diff} settings={settings} />}
...
function StandardLayout({ caseData, clues, phase, setPhase, foundClues, progress, activeRoom, setActiveRoom, discoverClue, notes, setNotes, settings, suspects, selSuspect, setSelSuspect, interrogHist, setInterrogHist, questionCounts, setQuestionCounts, dynamicAlibis, setDynamicAlibis, lieScores, setLieScores, crossState, setCrossState, witnessState, setWitnessState, subTab, setSubTab, player, diff, revSuspicion, hint, hintUsed, hintLoading, showHint, getHint, unlimitedHints, aiHints, setShowDossier, setShowTimeline, gameMode, isMobile }) {
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "18px 16px", display: isMobile ? "flex" : "grid", flexDirection: isMobile ? "column" : undefined, gridTemplateColumns: isMobile ? undefined : "240px 1fr", gap: 16 }}>
      {!isMobile && (
        <Sidebar caseData={caseData} foundClues={foundClues} clues={clues} progress={progress} revSuspicion={revSuspicion} hint={hint} showHint={showHint} hintUsed={hintUsed} hintLoading={hintLoading} getHint={getHint} unlimitedHints={unlimitedHints} aiHints={aiHints} openaiKey={settings.openaiKey} />
      )}
      <div>
        {!settings.openaiKey && <div style={{ marginBottom: 14 }}><APIWarn /></div>}
        {phase === "detective" && (
          <CorkboardPanel caseData={caseData} clues={clues} activeRoom={activeRoom} setActiveRoom={setActiveRoom} discoverClue={discoverClue} notes={notes} setNotes={setNotes} settings={settings} />
        )}
        {phase === "interrogation" && (
          <div className="anim-in">
            <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              <button className="btn btn-sm btn-ghost" style={{ marginLeft: "auto" }} onClick={() => setShowDossier(selSuspect || suspects[0])}>📋</button>
              <button className="btn btn-sm btn-ghost" onClick={() => setShowTimeline(selSuspect || suspects[0])}>⏱</button>
            </div>
            {subTab === "interrogate" && <InterrogationTab caseData={caseData} suspects={suspects} selSuspect={selSuspect} setSelSuspect={setSelSuspect} interrogHist={interrogHist} setInterrogHist={setInterrogHist} questionCounts={questionCounts} setQuestionCounts={setQuestionCounts} dynamicAlibis={dynamicAlibis} lieScores={lieScores} setLieScores={setLieScores} player={player} settings={settings} diff={diff} />}
            {subTab === "cross" && <CrossExamTab caseData={caseData} suspects={suspects} selSuspect={selSuspect} setSelSuspect={setSelSuspect} crossState={crossState} setCrossState={setCrossState} dynamicAlibis={dynamicAlibis} setDynamicAlibis={setDynamicAlibis} player={player} settings={settings} diff={diff} />}
            {subTab === "witnesses" && <WitnessTab witnesses={caseData.witnesses || []} witnessState={witnessState} setWitnessState={setWitnessState} player={player} settings={settings} />}
          </div>
        )}
      </div>
    </div>
...
// ── TV Layout (1400px+) ──────────────────────────────────────
function TVLayout({ caseData, clues, phase, setPhase, foundClues, progress, activeRoom, setActiveRoom, discoverClue, notes, setNotes, settings, suspects, selSuspect, setSelSuspect, interrogHist, setInterrogHist, questionCounts, setQuestionCounts, dynamicAlibis, setDynamicAlibis, lieScores, setLieScores, crossState, setCrossState, witnessState, setWitnessState, subTab, setSubTab, player, diff, revSuspicion, hint, showHint, hintUsed, hintLoading, getHint, unlimitedHints, aiHints, setShowDossier, setShowTimeline, gameMode }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "300px 1fr 280px", gap: 20, padding: "20px 28px", maxHeight: "calc(100vh - 120px)" }}>
        ...
        {gameMode === "combined" && (
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            {[["detective", "🔍", "Detect"], ["interrogation", "💬", "Interrogate"]].map(([id, icon, lbl]) => (
              <button key={id} className={`btn ${phase === id ? "btn-teal" : "btn-ghost"}`} style={{ fontSize: 14 }} onClick={() => setPhase(id)}>{icon} {lbl}</button>
            ))}
          </div>
        )}
        {!settings.openaiKey && <div style={{ marginBottom: 14 }}><APIWarn /></div>}
        ...
        {phase === "interrogation" && (
          <div className="anim-in">
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {[["interrogate", "💬", "Interrogate", "btn-gold"], ["cross", "⚔", "Cross-Exam", "btn-red"], ["witnesses", "👁", "Witnesses", "btn-teal"]].map(([id, icon, lbl, btn]) => (
                <button key={id} className={`btn ${subTab === id ? btn : "btn-ghost"}`} onClick={() => setSubTab(id)}>{icon} {lbl}</button>
              ))}
              <button className="btn btn-ghost" style={{ marginLeft: "auto" }} onClick={() => setShowDossier(selSuspect || suspects[0])}>📋</button>
              <button className="btn btn-ghost" onClick={() => setShowTimeline(selSuspect || suspects[0])}>⏱</button>
            </div>
            {subTab === "interrogate" && <InterrogationTab caseData={caseData} suspects={suspects} selSuspect={selSuspect} setSelSuspect={setSelSuspect} interrogHist={interrogHist} setInterrogHist={setInterrogHist} questionCounts={questionCounts} setQuestionCounts={setQuestionCounts} dynamicAlibis={dynamicAlibis} lieScores={lieScores} setLieScores={setLieScores} player={player} settings={settings} diff={diff} />}
            {subTab === "cross" && <CrossExamTab caseData={caseData} suspects={suspects} selSuspect={selSuspect} setSelSuspect={setSelSuspect} crossState={crossState} setCrossState={setCrossState} dynamicAlibis={dynamicAlibis} setDynamicAlibis={setDynamicAlibis} player={player} settings={settings} diff={diff} />}
            {subTab === "witnesses" && <WitnessTab witnesses={caseData.witnesses || []} witnessState={witnessState} setWitnessState={setWitnessState} player={player} settings={settings} />}
        ...
        {suspects.map(s => {
          ...
          return (
            <div key={s.id} className={`portrait-card ${selSuspect?.id === s.id ? "selected" : ""} ${cs.cracked ? "cracked" : ""}`}
              ...
              onClick={() => { setSelSuspect(s); if (phase !== "interrogation") setPhase("interrogation"); }}>
